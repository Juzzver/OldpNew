...
{
	public abstract class BaseBloodPotion : BasePotion
	{
		public abstract int MinHeal { get; }
		public abstract int MaxHeal { get; }

		public BaseBloodPotion( PotionEffect effect ) : base( 0xF0C, effect )
		{
			Hue = 1920;
		}

		public BaseBloodPotion( Serial serial ) : base( serial )
		{
		}
		...
		public override void Drink( Mobile from )
		{
			if ( from.Hits < from.HitsMax )
			{
				...
				{
					if ( from.BeginAction( typeof( BaseBloodPotion ) ) )
					{
						DoHeal( from );
		...
		{
			((Mobile)state).EndAction( typeof( BaseBloodPotion ) );
		}
	}
}
